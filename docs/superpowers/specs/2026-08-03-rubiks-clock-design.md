...
lib/scramble/             ScrambleProvider interface + cubing.js implementation
                          (lazy dynamic import, runs in a worker).
...
components/               TimerDisplay, ScrambleBar, CubePreview, KeyHints,
...
in competition) plus a 2D/3D preview of the scrambled cube, both loaded on
demand so they never block the first paint.
