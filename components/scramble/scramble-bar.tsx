...
import { CubeNet } from '@/components/scramble/cube-net'
...
export function ScrambleBar({
  ...
  return (
    <div className="flex w-full flex-col items-center gap-4 sm:flex-row sm:gap-6">
      ...
      {/* `sm:contents` dissolves this wrapper on wider screens, so the net and
        * the button become items of the strip itself. Below that they stay a row
        * of their own under the scramble, which keeps the phone layout two lines
        * tall instead of three. */}
      <div className="flex items-center gap-4 sm:contents">
        <div aria-hidden="true" className="size-24 shrink-0 sm:order-first sm:size-28">
          {!error && scramble ? <CubeNet scramble={scramble} /> : null}
        </div>
